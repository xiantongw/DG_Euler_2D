...
Param ReadParamIn(string param_fname)
{
	// variables dealing with strings
    ifstream param_file(param_fname);
    stringstream ss;
	...
	while (getline(param_file, line))
	{
		...
		}else if (strcasecmp(param_name.c_str(), "order_geo") == 0)
		{
			param.order_geo = int(atof(param_value.c_str()));
		}
	}
	param_file.close();
	return param;
